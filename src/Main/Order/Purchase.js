...
export const Purchase_List = ({ navigation }) => {
    ...
    const GetProduct = useCallback(async () => {
        try {
            ...
            const data = response?.data?.orderHistory?.filter(ele => ele?.orderItems?.some(item => item.productId?.warehouse === loginData?._id && item?.status == "ordered"));
            setProductData(data);
    ...
    const DeliverProduct = async (item) => {
        ...
        const damage = itemState.damage || 0;
        ...
        const invalidQtyItems = item.orderItems.filter(ele => {
            const receivedQty = ele.receivedQty;
            if (ele.productId.warehouse === loginId) {
                return receivedQty === undefined || isNaN(receivedQty) || receivedQty < 0;
            }
            return false;
        });

        const invalidDamageQty = item.orderItems.filter(ele => {
            const DamageQty = ele.damageQty;
            if (ele.productId.warehouse === loginId) {
                return DamageQty === undefined || isNaN(DamageQty) || DamageQty < 0;
            }
            return false;
        });

        if (invalidDamageQty.length > 0) {
            Alert.alert("Error!!!", "Please enter a valid number of damaged items");
            return;
        }
        if (invalidQtyItems.length > 0) {
            Alert.alert("Error!!!", "Please enter valid quantities for all products.");
            return;
        }
        ...
        // console.log("NoOfPackage:", noOfPackage);
        ...
        await axios.post(`${CustomerBaseUrl}purchase-order/dipatch-purchase-order/${item?._id}`, payload)
            .then((res) => {
                ...
                <Card.Content style={styles.content}>
                    ...
                    <View style={{
                        ...
                        <Text style={{
                            ...
                            CN No.: {item?.orderNo?.toUpperCase()}
                        </Text>
                    </View>
